...
cd backend
python main.py > /tmp/sam3-backend.log 2>&1 &
BACKEND_PID=$!
cd ..
...
cd frontend
npm run dev > /tmp/sam3-frontend.log 2>&1 &
FRONTEND_PID=$!
cd ..
...
echo ""
echo "💡 Logs:"
echo "   Backend:  tail -f /tmp/sam3-backend.log"
echo "   Frontend: tail -f /tmp/sam3-frontend.log"
echo ""
